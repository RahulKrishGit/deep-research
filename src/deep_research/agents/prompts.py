...
REPORT_INSTRUCTION = (
    ...
    "Two figures for one measurement that the checked claims attribute to "
    "the same issuer on different dates are editions of that issuer's "
    "series, not a conflict: state the later-dated figure as the current one "
    "and the earlier one as an earlier edition with only the date its claim "
    "carries, and never present the pair as a range or an unresolved "
    "disagreement. Never name an edition, inventory month, or release date "
    "that no claim carries.\n"
    "A figure whose claim names a segment or basis keeps it in the sentence, "
    "and a total covering more than the segment the question asks about is "
    "never stated as the answer to it. Do not spend a point on event "
    "logistics, or on a relay restating a figure an issuer's own claim "
    "already states.\n"
    ...
    "claim_ids: copy the labels exactly as printed in the evidence packet "
    "(such as C001). Every checked claim appears there exactly once, under "
    "one label; a label that is not in that packet is refused.\n"
