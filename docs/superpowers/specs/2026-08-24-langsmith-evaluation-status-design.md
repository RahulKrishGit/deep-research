...
judge-status feedback. It does not receive the final aggregate status, so the
Experiments view shows evidence of a failure but not the harness verdict.
...
4. The local artifact and CLI continue to use the same status calculation and
...
- Adding a dashboard, alert, or automation beyond the experiment feedback
  columns.
