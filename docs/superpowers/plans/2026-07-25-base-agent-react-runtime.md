...
**Post-review fix (2026-07-31): final whole-branch review findings.** The final pre-merge branch review (post-Task 7) found one blocking issue and one recommended follow-up in the code delivered by Tasks 5 and 6:

- **Blocking crash fix (Task 5, `parse_tool_input` in `deep_research/agents/steps.py`).** A model-supplied `tool_input_json` containing the literal key `"self"` (e.g. `{"self": "x"}`) reached `tool.execute(**tool_input)` unfiltered and raised `TypeError: BaseTool.execute() got multiple values for argument 'self'` *at the call site*, before `BaseTool.execute`'s own `try/except Exception` could convert it into a failed `ToolResult`. This propagated out of `run_react_loop` and `BaseAgent.run` uncaught — no `AgentRun`, no recorded error. `parse_tool_input` now rejects the key `"self"` and any key that is not a valid Python identifier (`str.isidentifier()`), raising `ValueError` so the loop's existing `except ValueError` → `agent_invalid_tool_input` observation path handles it, matching the docstring's stated contract ("raises `ValueError` for anything the runtime cannot forward to `BaseTool.execute(**kwargs)`"). Covered by `test_parse_tool_input_rejects_the_self_key`, `test_parse_tool_input_rejects_a_non_identifier_key` (`tests/test_agents/test_steps.py`), and `test_self_keyed_tool_arguments_are_a_recoverable_observation` (`tests/test_agents/test_react.py`), which pins that the loop does not crash and instead finishes normally with a recoverable observation.
- **Configurable observation summary length (Task 1 + Task 5 + Task 6).** `run_react_loop`'s `summary_limit` parameter existed but nothing ever set it below `DEFAULT_SUMMARY_LIMIT = 200`, and Researcher-style agents (spec 08) will need a wider or narrower budget per agent. Added `AgentRuntimeConfig.observation_summary_chars: int = Field(default=200, ge=1)` alongside `max_iterations`/`tool_budget`/`prompt_context_entries`, plus the matching `config.yaml` entry and `AGENTS_OBSERVATION_SUMMARY_CHARS` environment override. `BaseAgent.run` now passes `summary_limit=self._config.observation_summary_chars` into `run_react_loop`. Covered by config tests mirroring the existing `AGENTS_MAX_ITERATIONS`-style tests in `tests/test_config.py`, and by `test_run_uses_the_configured_observation_summary_limit` in `tests/test_agents/test_base.py`, which drives a real `agent.run(...)` with a 20-character limit and asserts the resulting observation summary is actually clamped.
- **Minor consistency fix (Task 5).** The unknown-tool branch in `run_react_loop` clamped the tool name in its `agent_error` *message* via `summarize_text` but put the raw, unbounded `tool_name` into `details={"tool": tool_name, ...}`, which flows into `ResearchState.errors` unclamped; the iteration span's `"tool"` output field had the same gap for `decision.tool_name`. Both now use `summarize_text(..., limit=summary_limit)`. Covered by `test_unknown_tool_error_details_clamp_an_oversized_tool_name` in `tests/test_agents/test_react.py`.
- **Deliberately not fixed.** Two low-severity findings from the same review were left as-is: the tool-budget-exhausted observation text reads as if the model will see and act on it, but the loop stops immediately after setting `stop_reason = "tool_budget_exhausted"` so no further `decide()` call ever renders it — a wording mismatch, not a bug, not worth a design discussion in an automated fix pass. The agent-span metric reports `success=True` even when the run stopped on `provider_error`, because `run_react_loop` already converted the `OpenAIProviderError` into a stop reason before `BaseAgent.run`'s span closes; this is arguably correct (the agent span itself didn't fail) but is a maintainer design call, not an automated fix.
