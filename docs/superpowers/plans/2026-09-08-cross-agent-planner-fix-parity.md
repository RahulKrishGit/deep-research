...
| 12. Evidence-gated agent quality/trajectory repair | **In progress — evaluator and Critic routing waves complete** | Commits `7c58abc` and `12f9ebe` repair cross-channel `required_fields_present` and typed Critic provider-fallback routing with focused regressions. Critic context wiring and typed judge diagnosis remain pending. |
